...
-- Bite 30G.2 introduced FINAL_TENANT_PAYMENT / FINAL_COLLABORATOR_PAYMENT
-- Journey Settlements and FINAL_SETTLEMENT Ledger Entries. The Go models and
-- service support those values, but the production SQLite CHECK constraints
-- still reflect the pre-30G.2 enums from migration 000018. Rebuild both tables
-- so the database contract matches the application contract while preserving
-- every existing column, index, tenant guard, and financial-owner guard.

CREATE TABLE ledger_entries_000061 (
  id TEXT PRIMARY KEY,
...
INSERT INTO ledger_entries_000061 (
...
ALTER TABLE ledger_entries_000061 RENAME TO ledger_entries;
...
CREATE TABLE journey_settlements_000061 (
  id TEXT PRIMARY KEY,
  created_at DATETIME NOT NULL,
  updated_at DATETIME NOT NULL,
  tenant_id TEXT NOT NULL DEFAULT 'default',
  ...
  CHECK (settlement_type IN (
    ...
    'PAYOUT',
    'CLOSE_JOURNEY',
    'FINAL_TENANT_PAYMENT',
    'FINAL_COLLABORATOR_PAYMENT'
  )),
...
INSERT INTO journey_settlements_000061 (
...
ALTER TABLE journey_settlements_000061 RENAME TO journey_settlements;
