...
`make deployed-playwright-evidence-check` locally regression-tests the evidence writer/verifier contract and is included in `make local-check`. Requirement 13 remains `pending-30L.4C` until Production consumes the exact Test evidence and produces final release evidence.
