...
The machine-readable final-coverage map is `docs/bite-30l4-coverage-manifest.json`. `make bite30l4-coverage-manifest-check` validates every architecture requirement and its named repository evidence, and `make local-check` invokes that manifest verifier before the canonical local test/build gate.

## Bite 30L.4B — Deployed Development/Test Verification Evidence

Development and Test deployments now require deployed Playwright rather than allowing the verification step to be skipped. The deployment resolves a commit SHA and tree SHA, verifies the server checkout against both, and the deployed Playwright job independently verifies that its own checkout matches both identifiers. A successful run emits `deployment-evidence/deployed-playwright-verification.json`, verifies it against the exact environment/revision/tree, and uploads it under an immutable source/run-qualified artifact name together with similarly qualified Playwright report/test-results artifacts. The evidence SHA-256 and artifact identity are recorded into successful Test release-rehearsal markers for later Production gating.

`make deployed-playwright-evidence-check` locally regression-tests the Development/Test evidence writer/verifier contract and is included in `make local-check`. 30L.4C adds `make production-release-evidence-check`, requires the exact Test tree/evidence binding before Production, and emits final Production deployment/public-smoke evidence. Requirement 13 is therefore covered and the manifest is complete at 13/13.
