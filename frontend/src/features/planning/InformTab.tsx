  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold">{t("planning.informCollaborators")}</h2>
          <p className="text-sm text-gray-500">
            {t("planning.informHelp")}
          </p>
        </div>
        <div className="flex gap-2">
          {canInform && (
            ...
            >
              {props.pending ? t("planning.informing") : t("planning.markInformed")}
            </button>
          )}
          <button
      ...
      {unreplacedAbsentees.length > 0 && (
        ...
        >
          <p className="font-semibold">
            {t("planning.timeOffWarning")}
          </p>
          <p className="mt-1">
            {t("planning.timeOffHelp")}
          </p>
      ...
      <div className="rounded-2xl border bg-white p-5 shadow-sm print:border-0 print:shadow-none">
        {props.loading ? (
          <p>{t("planning.loadingRoster")}</p>
        ) : (
          <>
            <div className="border-b pb-4 text-center">
              <h3 className="text-2xl font-bold">
                {props.roster?.title ?? t("planning.workPlan")}
              </h3>
              <p className="mt-1 text-sm text-gray-600">
                {props.roster?.subtitle ??
                  `${formatDate(props.workPeriod.workDate)} — ${props.workPeriod.name}`}
              </p>
            ...
            <table className="mt-4 w-full text-left text-sm">
              <thead>
                <tr className="border-b">
                  <th className="py-2">{t("planning.name")}</th>
                  <th className="py-2">{t("collaborator.sector")}</th>
                  <th className="py-2">{t("planning.local")}</th>
                  <th className="py-2">{t("collaborator.task")}</th>
                </tr>
              </thead>
              <tbody>
            ...
            {(props.roster?.rows.length ?? 0) === 0 && (
              <p className="py-8 text-center text-sm text-gray-500">
                {t("planning.noIncludedCollaborators")}
              </p>
            )}
          </>
        )}
