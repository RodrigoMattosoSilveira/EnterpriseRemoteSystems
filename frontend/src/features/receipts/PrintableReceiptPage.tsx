...
import { usePrintableReceipt, usePrintReceipt } from "./useReceipt";
      ...
      <section className="mx-auto max-w-3xl rounded-2xl bg-white p-8 shadow-sm print:rounded-none print:shadow-none">
        <div className="print:hidden">
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <button type="button" disabled={!authorizedBy.trim() || printReceipt.isPending} onClick={print} className="rounded-xl bg-gray-900 px-4 py-2 font-semibold text-white disabled:opacity-50">
              {printReceipt.isPending ? "Preparing..." : "Print Receipt"}
            </button>
          </div>
          {printReceipt.error ? <div className="mt-3"><ApiErrorPanel error={printReceipt.error} /></div> : null}
        </div>
